...
public interface ConstantInterface {
    ...
    /**
     * FIELD_CONSTANT: a normal (class) field LOCAL_VARIABLE: some variable
     * inside a method MATH_OPCODE_CONSTANT: some constant inside a math opcode
     * ARRAY: if this is set, all values from an array are aggregated inside one
     * constant LOCAL_UNREFERENCED_CONSTANT: some constant which are not
     * assigned to any variable, eg, i = i+1. EXTERNAL_METHOD: An
     * unknown/external (api) method was invoked and the result was written to
     * the backtracked register, methods will only be threated this way if the
     * search is not fuzzy INTERNAL_BYTECODE_OP: Something unusual, if, eg, some
     * exception got moved to our backtracked register! UNCALLED_METHOD: A
     * method which is never directly invoked but has parameters linked to a
     * tracked register, eg, android.content.BroadcastReceiver.receive(..).
     * Might also be dead code.
     */
    ...
    /**
     * The different primitive types in Java and some "special" types: "String",
     * "Math_OP", "Other_Class" (for classes) and "Unknown".
     */
    public enum Type {
