...
/**
 * This class represents one line of code. The codeline will be stripped of all
 * leading and trailing whitespace and unprintable bytes in the range of 0 to
 * 32.
 */
public class CodeLine implements CodeLineInterface {
    ...
    /**
     * Deletes all whitespace and non printable bytes (bytes <= 32) from the
     * beginning and the end of the byte array @param line @return
     */
    private static byte[] trim(byte[] line) {
        int begin = 0;
        while (begin < line.length && line[begin] <= 32) {
            begin++;
        }

        int end = line.length - 1;
        while (end >= 0 && line[end] <= 32) {
            end--;
        }
        ...
        if (end < begin) {
            ...
            return Arrays.copyOfRange(line, begin, end + 1);
        }
    }
