...
/**
 *
 * Scans the given file (directory) recursively for files matching the given
 * filter string.
 */
public class FileList {
    ...
    /**
     * Get the used suffix to filter files. If no suffix was used, an empty
     * string is returned.
     *
     * @return
     */
    ...
    /**
     * Scan all files. This method will separate files from ad package on its
     * own, see getAllFoundFiles(boolean includeFilesFromAdPackages).
     *
     * @param file the directory to scan or a single file to add
     * @param suffix the filter for file names, only files ending with suffix
     * are added (maybe null or empty and the check is NOT case sensitive)
     */
    ...
    /**
     * Fill the internal vector with all found files. This method calls itself
     * recursively.
     *
     * @param file The file or directory to start with.
     * @param suffix the suffix for files which shall be included
     * @param isAdPackage denotes whether the file is from an ad package, must
     * be false when called
     */
