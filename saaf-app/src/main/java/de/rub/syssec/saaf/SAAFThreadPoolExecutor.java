 ...
 *
 * You have to call the SAAFThreadPoolExecutor.shutdown() after all threads are
 * submitted to the pool, because the terminate function is called so you can
 * react after that.
 *
...
class SAAFThreadPoolExecutor extends ThreadPoolExecutor {
    ...
    /**
     * A ThreadPoolExecutor which will automatically start all analyzes for all
     * given files.
     *
    ...
    protected void afterExecute(Runnable r, Throwable t) {
        ...
        if (errorOccured && Config.getInstance().getBooleanConfigValue(ConfigKeys.ANALYSIS_QUIT_ON_ERROR)) {
            ...
            LOGGER.error("Something unexpected occurred, see above.");
            // Will now perform an unclean Shutdown (DB connections are not closed etc, but ShutdownHooks are run)!
        }
    }
