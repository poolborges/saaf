...
public class Instruction implements InstructionInterface {
     ...
     * cmpr[1] the method, cmpr[2] the raw parameters and cmpr[3] the return
     * value.
     */
    ...
    public void parseOpCode() {
        ...
        if (!(type == InstructionType.EMPTY_LINE
                || type == InstructionType.SMALI_DOT_COMMENT
                || type == InstructionType.LABEL
                || type == InstructionType.SMALI_HASH_KEY_COMMENT || type == InstructionType.UNKNOWN // do not ask the map if we know it does not begin w/ a lowercase letter
                )) {
    ...
    private void parse(LinkedList<byte[]> split) {
        ...
        switch (type) {
            ...
            case AGET:
                /**
                 * aget-object v0, v0, v1 arrayop vAA, vBB, vCC Store data from
                 * array vBB at index vCC into vAA
                 *
            ...
            case GET:
                /**
                 * iinstanceop vA, vB, field@CCCC iget-x sstaticop vAA,
                 * field@BBBB sget-x sget-object v1,
            ...
            case APUT:
                /**
                 * Put data from vAA into the array vBB at index vCC arrayop
                 * vAA, vBB, vCC aput-x
                 *
            ...
            case PUT:
                /**
                 * Save vA in field CCCC of Object vB iinstanceop vA, vB,
                 * field@CCCC iput-x sstaticop vAA, field@BBBB sput-x sput v0,
...
//					+ ": " + new String(opCode));
            ...
            default:
                if (DEBUG) {
                    LOGGER.debug("Did not parse instruction of type " + type + ": "
                            + new String(opCode));
                }
                break;
        }
