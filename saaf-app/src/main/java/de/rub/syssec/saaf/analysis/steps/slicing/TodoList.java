...
/**
 *
 * This class is a helper class for the DetectionLogic and holds "jobs" which
 * need to be processed at a later time in the program slicing process.
 *
...
public class TodoList {
     ...
     * @param path
     * @return true if this was not yet searched through, create a new object,
     * you'll get inconsistencies if you'll reuse the same object
     */
    public boolean addReturnValuesFromMethod(byte[][] cm, int fuzzyLevel, int fuzzyLevelOffset, LinkedList<BasicBlockInterface> path) {
        if (DEBUG) {
            LOGGER.debug(" -> Add RETURN VALUE: " + new String(cm[0]) + "." + new String(cm[1]) + "\tfuzzy=" + fuzzyLevel + "/" + fuzzyLevelOffset);
        }
        ...
        if (returnMap.contains(ctt)) {
            return false;
        }
        if (returnMapDone.contains(ctt)) {
            return false;
        }
        returnMap.add(ctt);
        return true;
    ...
    public boolean addField(byte[][] cf, int fuzzyLevel, int fuzzyLevelOffset, LinkedList<BasicBlockInterface> linkedList) {
        if (DEBUG) {
            LOGGER.debug(" -> Add FIELD: " + new String(cf[0]) + "." + new String(cf[1]) + "\tfuzzy=" + fuzzyLevel + "/" + fuzzyLevelOffset);
        }
        if (fuzzyLevel + fuzzyLevelOffset > MAX_FUZZY_LEVEL) {
            if (DEBUG) {
                LOGGER.debug("    Maximum fuzzy level reached (" + MAX_FUZZY_LEVEL + "): aborting.");
        ...
        if (fieldMap.contains(ctt)) {
            return false;
        }
        if (fieldMapDone.contains(ctt)) {
            return false;
        }
        fieldMap.add(ctt);
        return true;
    ...
    public static class RegisterSearch {
         ...
         * @param index the index where to start backtracking inside the
         * {@linkplain BasicBlock}
         * @param fuzzyLevel set this to >1 if the search gets noisy, eg, if you
         * are backtracking into the blue for unknown method calls and are
         * interested in the parameters from such a call
         * @param fuzzyOffset
         * @param path the path through the BBs of this search, create a new
         * object, you'll get inconsistencies if you'll reuse the same object
         */
        ...
        /**
         * This method sets an offset to the fuzzy value. If the offset plus the
         * fuzzy is too high, the search will be aborted.
         *
         * @param offset
         */
    ...
    private static boolean checkRsForEquality(RegisterSearch rs1, RegisterSearch rs2) {
        if (!Arrays.equals(rs1.getRegister(), rs2.getRegister())) {
            return false;
        }
        if (rs1.getBB() != rs2.getBB()) {
            return false;
        }
        if (rs1.getIndex() != rs2.getIndex()) {
            return false;
        }
        return true;
    }
    ...
    public boolean addRegisterToTrack(RegisterSearch rs) {
        if (DEBUG) {
            LOGGER.debug(" -> Add REGISTER: " + new String(rs.getRegister()) + ", " + rs.getBB().getMethod().getName() + ":" + rs.getIndex() + "\tfuzzy=" + rs.getFuzzyLevel() + "/" + rs.getFuzzyOffset() + ", bb=" + rs.getBB().getUniqueId());
        }
        ...
        for (RegisterSearch rs2 : regListDone) {
            if (checkRsForEquality(rs, rs2)) {
                if (DEBUG) {
                    LOGGER.debug("     Already searched this RS. It will be ignored! (This is ok)");
                }
                return false;
            }
        }
        ...
        if (!found) {
            regList.addFirst(rs); // this way we first work up actual stuff
        } else if (DEBUG) {
            LOGGER.debug("     Duplicate RS added. It will be ignored! (This is ok)");
        }
        return true;
    }
    ...
    /**
     * A helper class to identify content of a given class. This may be an array
     * name, a method or a field name.
     */
    public static class ClassContentTracker {
        ...
        /**
         * This is a helper class to wrap some content. It can contain arbitrary
         * data, but the first entry of the first parameter must always be the
         * full class name and the second one the identifier of a method, array
         * or field. Further entries are optional and can contain, eg, the
         * parameters for a method and it's return value.
         *
         * @param ci [class, identifier, more optional]
         * @param fuzzyLevel
         * @param fuzzyLevelOffset the offset to the fuzzyLevel
         * @param create a new object, you'll get inconsistencies if you'll
        ...
        /**
         * Get the class and identifier of the class content (array, method or
         * field name) and other optional data.
         *
         * @return ci, ci[0]=full class name, ci[1]=identifier (ci[2+] are
        ...
        /**
         * The path through the program for this search, the list is always a
         * new copy of the original list. Changes to the returned list will not
         * effect the internal list!
         *
         * @return the path
         */
        public LinkedList<BasicBlockInterface> getPath() {
        ...
        public boolean equals(Object other) {
            ...
            if (ci.length != ((ClassContentTracker) other).getCi().length) {
                return false;
            }
            for (int i = 0; i < ci.length; i++) {
                if (!(Arrays.equals(ci[i], ((ClassContentTracker) other).getCi()[i]))) {
                    return false;
                }
            }
            return true;
     ...
     * @param fuzzyLevel
     * @param path, create a new object, you'll get inconsistencies if you'll
     * reuse the same object
    ...
    public boolean addArrayFieldToTrack(byte[][] ca, int fuzzyLevel, int fuzzyLevelOffset, LinkedList<BasicBlockInterface> path) throws SyntaxException {
        if (DEBUG) {
            LOGGER.debug(" -> Add ARRAY FIELD: " + new String(ca[0]) + "." + new String(ca[1]) + "\tfuzzy=" + fuzzyLevel + "/" + fuzzyLevelOffset);
        }
        if (fuzzyLevel + fuzzyLevelOffset > MAX_FUZZY_LEVEL) {
            if (DEBUG) {
                LOGGER.debug("    Maximum fuzzy level reached (" + MAX_FUZZY_LEVEL + "): aborting.");
        ...
        if (arrayMap.contains(ctt)) {
            return false;
        }
        if (arrayMapDone.contains(ctt)) {
            return false;
        }
        arrayMap.add(ctt);
        return true;
