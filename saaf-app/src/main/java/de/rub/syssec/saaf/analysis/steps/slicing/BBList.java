...
public class BBList {
    ...
    /**
     * Holds information about the current path, eg, what register is tracked
     * where in the path
     */
    ...
    /**
     * Create a new list of BBs which automatically handles the path for each BB
     * which is found by a depth-first search.
     *
     * @param firstBB the first BB of the new path, it will be returned on the
     * first call of getNextBB()
     * @param pathState Can be used to denote the tracked register for the
     * current BB, it can be overridden if it changes while handling new BBs
     * @param isBackwardSearch are BBs searched forward (next) of backwards
     * (previous)
     */
    ...
    /**
     * Create a new list of BBs which automatically handles the path for each BB
     * which is found by a depth-first search. This method will store the
     * previous path and will search for other BBs from the last BB in the path.
     *
     * @param previousPath the actual BB is the last BB in the given path, only
     * the last BB will be returned on the first call to getNextBB().
     * @param pathState Can be used to denote the tracked register for the
     * current BB, it can be overridden if it changes while handling new BBs
     * @param isBackwardSearch are BBs searched forward (next) or backwards
     * (previous)
     */
    ...
    /**
     * Create a new list of BBs which automatically handles the path for each BB
     * which is found by a depth-first search. The search is conducted
     * backwards.
     *
     * @param firstBB the first BB of the new path, it will be returned on the
     * first call of getNextBB()
    ...
    /**
     * Returns BBs which are found during the DFS until no new BBs can be found.
     * The first BB will always be returned on the first call, see constructor.
     * The path is automatically stored and the current one can be retrieved
     * with the getPathForLastBB() method.
     *
    ...
    public BasicBlockInterface getNextBb() {
        ...
        while (!path.isEmpty() && retBB == null) { // search until retBB is found or no more BBs are available
            ...
            if (isBackwardSearch) {
                blocks = current.getPreviousBB();
            } else {
                blocks = current.getNextBB();
            }
    ...
    /**
     * Remove the last BB from the current path and search for the next BB
    ...
    /**
     * Add a new state to the current BB in the path, it will be propagated to
     * the next (previous) BBs in the path.
     *
     * @param state
     */
